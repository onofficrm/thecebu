<?php
/**
 * 1회성 Phase2b — event 게시판 + 샘플 이벤트 (실행 후 삭제)
 */
...
if (!hash_equals('thecebu.co.kr-phase2b-20260524', $key)) {
...
foreach (eottae_install_get_board_defs() as $def) {
    if ($def['bo_table'] !== 'event') {
        continue;
    }
    $logs[] = eottae_install_create_board($def);
}

foreach (eottae_seed_sample_events() as $entry) {
    $logs[] = $entry;
}
