...
$market_share_url = eottae_market_view_url((int) $view['wr_id']);
$market_has_map = $market_map_show && $market_lat !== '' && $market_lng !== '' && is_numeric($market_lat) && is_numeric($market_lng);
$shop_map = array(
...
$market_inquiry_phone = !empty($market_contact['use_phone']) ? trim((string) ($market_contact['phone'] ?? '')) : '';
$market_inquiry_opts = array(
    'phone'       => $market_inquiry_phone,
    'owner_mb_id' => $market_seller_mb_id,
    'shop_name'   => get_text($view['wr_subject']),
    'lat'         => $market_lat,
    'lng'         => $market_lng,
    'address'     => $market_location,
    'share_url'   => $market_share_url,
);

if ($market_has_map && function_exists('eottae_enqueue_google_maps')) {
...
<div class="market-view board-wrap board-wrap--eottae-market" id="bo_v" style="width:<?php echo $width; ?>">
    ...
    <article class="market-view__article">
        ...
        <aside class="market-view__aside">
            ...
            <?php if ($market_has_map) {
                include G5_PATH.'/components/eottae/shop-detail-map.php';
            } ?>

            <?php eottae_render_inquiry_buttons('detail', $market_inquiry_opts); ?>

            <?php
            $market_detail_sidebar_exclude_wr_id = (int) $view['wr_id'];
            include G5_PATH.'/components/eottae/market-detail-sidebar.php';
            ?>
        </aside>
    </article>

    <?php eottae_render_inquiry_buttons('mobile-bar', $market_inquiry_opts); ?>
