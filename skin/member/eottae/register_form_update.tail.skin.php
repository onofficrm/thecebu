<?php
if (!defined('_GNUBOARD_')) {
    exit;
}

if (!is_file(G5_LIB_PATH.'/eottae-member-profile.lib.php')) {
    return;
}

include_once G5_LIB_PATH.'/eottae-member-profile.lib.php';

$mb_id = isset($mb_id) ? preg_replace('/[^a-z0-9_@.-]/i', '', (string) $mb_id) : '';
if ($mb_id === '' || !eottae_member_profile_enabled()) {
    return;
}

$append_msg = function ($message) {
    global $msg;

    if ($message === '') {
        return;
    }
    if (!isset($msg)) {
        $msg = '';
    }
    $msg .= ($msg !== '' ? "\\r\\n" : '').$message;
};

$ai_tmp = isset($_POST['eottae_mb_img_ai']) ? basename(preg_replace('/[^a-zA-Z0-9._-]/', '', (string) $_POST['eottae_mb_img_ai'])) : '';
$has_upload = isset($_FILES['mb_img']) && is_uploaded_file($_FILES['mb_img']['tmp_name']);
$delete_requested = !empty($_POST['del_mb_img']);

if ($delete_requested) {
    $path = eottae_member_profile_image_path($mb_id);
    if ($path !== '' && is_file($path)) {
        @unlink($path);
    }
} elseif ($has_upload) {
    $apply = eottae_member_profile_apply_image_file($mb_id, $_FILES['mb_img']['tmp_name']);
    if (empty($apply['ok'])) {
        $append_msg(isset($apply['message']) ? (string) $apply['message'] : '프로필 사진 저장에 실패했습니다.');
    }
} elseif ($ai_tmp !== '') {
    $apply = eottae_member_profile_apply_ai_tmp($mb_id, $ai_tmp);
    if (empty($apply['ok'])) {
        $append_msg(isset($apply['message']) ? (string) $apply['message'] : 'AI 프로필 사진 저장에 실패했습니다.');
    }
}
