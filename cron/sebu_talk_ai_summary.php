 ...
 *
 * CLI:
 *   php cron/sebu_talk_ai_summary.php
 *   php cron/sebu_talk_ai_summary.php --dry-run
 *   php cron/sebu_talk_ai_summary.php --room-id=3
 *
 * 웹 (talkroom_ai_cron_key 설정 시):
 *   /cron/sebu_talk_ai_summary.php?key=YOUR_SECRET
 */
...
if ($is_cli) {
    ...
    $_SERVER['REQUEST_URI'] = '/cron/sebu_talk_ai_summary.php';
    $_SERVER['SCRIPT_NAME'] = '/cron/sebu_talk_ai_summary.php';
...
foreach ($result['results'] as $row) {
    ...
    if (!empty($row['activity'])) {
        echo '  activity: '.(int) $row['activity']."\n";
    }
